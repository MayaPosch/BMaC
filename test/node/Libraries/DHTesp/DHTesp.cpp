...
{
  readSensor();
  if ( error == DHT_ERROR_TIMEOUT ) { // Try a second time to read
    readSensor();
  }
  return humidity;
}
...
{
  switch ( error ) {
    case DHTesp::DHT_ERROR_TIMEOUT:
      return "TIMEOUT";

    case DHTesp::DHT_ERROR_CHECKSUM:
      return "CHECKSUM";
